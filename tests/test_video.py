...
class TestExtractFrame:
    ...
    def test_different_timestamps_differ(self, sample_video_path):
        f1 = extract_frame(sample_video_path, 30.0)
        f2 = extract_frame(sample_video_path, 120.0)
...
class TestSampleFrames:
    def test_yields_timestamp_frame_pairs(self, sample_video_path):
        ...
        # 0s and 30s (60s is < 61 so included)
        assert len(frames) == 3
    ...
    def test_start_end_range(self, sample_video_path):
        frames = list(
            sample_frames(sample_video_path, interval_sec=10.0, start_sec=100.0, end_sec=121.0)
        )
